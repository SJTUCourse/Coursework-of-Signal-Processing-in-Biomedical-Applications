	...
	: QDialog(parent)
{
    ...
    ui.txtPointCount1->setValidator(new QIntValidator(0, 10000000, this));
    ui.txtPointCount2->setValidator(new QIntValidator(0, 10000000, this));
...
void ConfigureDialog::DeviceChanged(int index)
{
    ...
    for (int i = 0; i < channelCount; i++)   /// The maximum number of channel is 2
	{
...
void ConfigureDialog::ButtonOKClicked()
{
    ...
    configure.pointCountPerWave1 = ui.txtPointCount1->text().toInt();
    configure.pointCountPerWave2 = ui.txtPointCount2->text().toInt();
